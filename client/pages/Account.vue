<template>
    <div class="page-content">
        <div class="d-flex align-items-center mb-3">
            <h4 class="page-title"><i class="bx bx-credit-card"></i>{{$t('page.account')}}</h4>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
            ...
            <div class="p-2" >
                <div class="card card-body">
                    <form action="" class="row g-3" method="post" @submit.prevent="submit()">
                        <h4 class="modal-title">{{$t(`page.${form_mode}`)}} {{$t('page.account')}}</h4>
                        <button type="button" class="btn-close" @click="$modal.hide('account_modal')"></button>
                        <div class="col-12">
                            <hr class="mt-0 mb-3" />
                        ...
                        <div class="col-12">
                            ...
                            <select class="form-select mt-1" v-model="form.company_id" required>
                        ...
                        <div class="text-right mt-3">
                            <hr class="mt-0 mb-3" />
                            <button type="button" class="btn btn-danger" @click="$modal.hide('account_modal')"><i class="bx bx-window-close"></i> {{$t('page.close')}}</button>
...
export default {
    middleware: 'auth',
    head () {
        return { title: this.$t('page.account') }
    },
    data() {
        return {
            data: [],
    ...
    methods: {
        ...
        submit() {
            let uri = this.form_mode == 'edit' ? '/account/update' : '/account/create';
            this.form.post(uri).then(response => {
                this.searchData();
                this.form.reset()
                this.$modal.hide('account_modal')
        ...
        remove(item, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.axios.get(`/account/delete/${item.id}`).then(response => {
