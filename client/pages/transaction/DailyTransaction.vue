<template>
    <div class="page-content">
        ...
        <div class="d-md-flex align-items-center mb-3">
            <h4 class="page-title mb-2 mb-md-0"><i class="bx bx-notepad"></i>{{ $t('page.daily_transaction') }}</h4>
                    ...
                    <div class="card-body p-3">
                        <div class="page-filter d-md-flex align-items-center">
                            <form action="" class="top-filter" @submit.prevent="searchForm()">
                                ...
                                <input v-model="filter.keyword" type="text" class="form-control fixed-width"
                                    :placeholder="$t('page.keyword')" />
                                <div class="input-group flex-nowrap fixed-width date-selector">
                                    <span class="input-group-text" @click="changeDate(-1)">&lt;</span>
                                    <datepicker v-model="filter.date" input-class="form-control" format="yyyy-MM-dd"
                                        :use-utc="true" :typeable="true" :placeholder="$t('page.date')"></datepicker>
                                    <span class="input-group-text" @click="changeDate(1)">&gt;</span>
                                </div>
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr v-for="(item, index) in data" :key="index">
                                        ...
                                        <td
                                            :class="{ 'text-danger': item.balance < 0, 'text-success': item.balance >= 0 }">
                                            {{ item.balance | currency }}</td>
        ...
        <modal name="transaction_modal" width="600" height="auto" :scrollable="true" :adaptive="true">
            <div class="p-3">
                <transaction-form :mode="form_mode" :transaction="selected_transaction"
                    @submitted="transactionSubmitted" @start-uploading="startUploading" @reloadPage="searchForm" @updateImage="updateImage" :categoriesForm="categoriesForm" />
            </div>
        </modal>
    </div>
</template>
...
export default {
    ...
    head() {
        return { title: this.$t('page.daily_transaction') }
    },
    data() {
        return {
            accounts: [],
    ...
    methods: {
        ...
        async getLastTransactionDate() {
            const response = await this.axios.get('/transaction/get_last_transaction_date')
            this.filter.date = response.data.data;
            this.searchForm();
        },
        ...
        initFirebase() {
            if (this.$isSubAdmin()) {
                this.$fire.database.ref('transactions/approved/').remove();
                const approvedTransactionsRef = this.$fire.database.ref(`transactions/${this.auth_user.company_id}/approved`).limitToLast(1);
                approvedTransactionsRef.on('value', (snapshot) => {
                    snapshot.forEach((doc) => {
                        // Admin approved a transaction
                        const payload = doc.val();
                        const index = this.data.findIndex(i => i.id === payload.transaction_id);
                        if (index > -1) {
        ...
        transactionSubmitted(result) {
            this.filter.date = this.$moment(result.timestamp).format('YYYY-MM-DD');
            this.searchForm();
        },
        async getTotal() {
        ...
        edit(item) {
            console.log(this.latest_audit_date);
            console.log(item.timestamp);
            if (item.is_audited || this.latest_audit_date>item.timestamp) {
        ...
        changeDate(days) {
            let result = this.$moment(this.filter.date).add(days, 'days').format('YYYY-MM-DD');
            this.filter.date = result
            this.searchForm();
        },
        resetFilter() {
            let filter_date = this.filter.date;
            this.filter = {
                page: 1,
                date: filter_date,
                company_id: '',
